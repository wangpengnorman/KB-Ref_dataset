...
class data(Dataset):
    ...
    def __getitem__(self, item):
        ...
        expression = np.pad(np.array(expression), (0, 50-self.e_mask[item]), 'constant', constant_values=(0, 9999))
        ...
        for i in range(len(self.mask[item])):
            middle = []
            l = 0
            for j in range(len(self.mask[item][0])):
        ...
        e_mask = np.pad(np.ones(self.e_mask[item]), (0, 50-self.e_mask[item]))
        e_mask = torch.from_numpy(e_mask).type(torch.FloatTensor)
        c_mask = np.pad(np.ones(self.c_mask[item]), (0, 10-self.c_mask[item]))
...
if __name__ == '__main__':
    ...
    a = data('./image','./json/N_A_test.json', data_transform=test_transform)
    jpg, label, f_label, expression, e_mask, local, locations, facts, mask, jpg_name = a.__getitem__(0)
